...
rem ----------------------------------------
if not defined CB_ROOT set CB_ROOT=%CD%\build_onlyBatch
rem ------------------------------------------
...
set PATH=%CB_ROOT%;%GCC_ROOT%\bin;%PATH%
...
if not defined START_CMD set START_CMD=start "codeAdapt Core MSys2 Build (wx 3.0.x)" /D"%~dp0" /min
set CB_EXE="%CB_ROOT%\codeadapt.exe"
if not defined CB_PARAMS set CB_PARAMS=-p msys2_clang --batch-build-notify --no-batch-window-close
set CB_CMD=%BUILD_TYPE% "%~dp0codeAdapt_wx30-msys2.cbp"
