...
#endif
    m_searchPaths.Add(wxT_2("fonts"));
    m_searchPaths.AddEnvList(wxT_2("WXPDF_FONTPATH"));
  }
...
wxPdfFontManagerBase::FindFile(const wxString& fileName, wxString& fullFileName) const
{
  ...
  if (myFileName.IsOk())
  {
    ...
    {
      wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::FindFile: ")) +
...
wxPdfFontManagerBase::RegisterFont(const wxString& fontFileName, const wxString& aliasName, int fontIndex)
{
  ...
  if (FindFile(fontFileName, fullFontFileName))
  {
    ...
    if (ext.IsSameAs(wxT_2("ttf")) || ext.IsSameAs(wxT_2("otf")) || ext.IsSameAs(wxT_2("ttc")))
    {
#if wxUSE_UNICODE
      ...
      if (fontData != NULL)
      {
        ...
        {
          delete fontData;
          wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::RegisterFont: ")) +
                     wxString::Format(_("Font file '%s' already registered."), fontFileName.c_str()));
        }
      }
#else
      wxLogError(wxString(wxT_2("wxPdfFontManagerBase::RegisterFont: ")) +
                 wxString::Format(_("Format of font file '%s' not supported."), fontFileName.c_str()));
#endif
    }
    else if (/* ext.IsSameAs(wxT_2("pfa")) || */ ext.IsSameAs(wxT_2("pfb")) || ext.IsEmpty())
    {
    ...
    else if (ext.IsSameAs(wxT_2("xml")))
    {
...
// TODO: Would testing for __WXGTK__ be sufficient?
  ...
  // Check font slant
  if (fontDesc.Find(wxT_2("Oblique")) != wxNOT_FOUND)
    slant = FC_SLANT_OBLIQUE;
  else if (fontDesc.Find(wxT_2("Italic")) != wxNOT_FOUND)
    slant = FC_SLANT_ITALIC;
  else
    slant = FC_SLANT_ROMAN;

  // Check font weight
  if (fontDesc.Find(wxT_2("Book")) != wxNOT_FOUND)
    weight = FC_WEIGHT_BOOK;
  else if (fontDesc.Find(wxT_2("Medium")) != wxNOT_FOUND)
    weight = FC_WEIGHT_MEDIUM;
#ifdef FC_WEIGHT_ULTRALIGHT
  else if (fontDesc.Find(wxT_2("Ultra-Light")) != wxNOT_FOUND)
    weight = FC_WEIGHT_ULTRALIGHT;
#endif
  else if (fontDesc.Find(wxT_2("Light")) != wxNOT_FOUND)
    weight = FC_WEIGHT_LIGHT;
  else if (fontDesc.Find(wxT_2("Semi-Bold")) != wxNOT_FOUND)
    weight = FC_WEIGHT_DEMIBOLD;
#ifdef FC_WEIGHT_ULTRABOLD
  else if (fontDesc.Find(wxT_2("Ultra-Bold")) != wxNOT_FOUND)
    weight = FC_WEIGHT_ULTRABOLD;
#endif
  else if (fontDesc.Find(wxT_2("Bold")) != wxNOT_FOUND)
    weight = FC_WEIGHT_BOLD;
  else if (fontDesc.Find(wxT_2("Heavy")) != wxNOT_FOUND)
    weight = FC_WEIGHT_BLACK;
  else
    weight = FC_WEIGHT_NORMAL;

  // Check font width
  if (fontDesc.Find(wxT_2("Ultra-Condensed")) != wxNOT_FOUND)
    width = FC_WIDTH_ULTRACONDENSED;
  else if (fontDesc.Find(wxT_2("Extra-Condensed")) != wxNOT_FOUND)
    width = FC_WIDTH_EXTRACONDENSED;
  else if (fontDesc.Find(wxT_2("Semi-Condensed")) != wxNOT_FOUND)
    width = FC_WIDTH_SEMICONDENSED;
  else if (fontDesc.Find(wxT_2("Condensed")) != wxNOT_FOUND)
    width = FC_WIDTH_CONDENSED;
  else if (fontDesc.Find(wxT_2("Ultra-Expanded")) != wxNOT_FOUND)
    width = FC_WIDTH_ULTRAEXPANDED;
  else if (fontDesc.Find(wxT_2("Extra-Expanded")) != wxNOT_FOUND)
    width = FC_WIDTH_EXTRAEXPANDED;
  else if (fontDesc.Find(wxT_2("Semi-Expanded")) != wxNOT_FOUND)
    width = FC_WIDTH_SEMIEXPANDED;
  else if (fontDesc.Find(wxT_2("Expanded")) != wxNOT_FOUND)
  ...
  {
    wxLogWarning(wxString(wxT_2("wxPdfFontManagerBase::RegisterFont: ")) +
...
wxPdfFontManagerBase::RegisterFontCollection(const wxString& fontCollectionFileName)
{
  ...
  if (FindFile(fontCollectionFileName, fullFontCollectionFileName))
  {
    ...
    if (fileName.IsOk() && fileName.GetExt().Lower().IsSameAs(wxT_2("ttc")))
    {
      wxPdfFontParserTrueType fontParser;
    ...
    {
      wxLogWarning(wxString(wxT_2("wxPdfFontManagerBase::RegisterFontCollection: ")) +
                   wxString::Format(_("Font collection file '%s' has not the file extension '.ttc'."), fontCollectionFileName.c_str()));
    }
  }
  else
  {
    wxLogError(wxString(wxT_2("wxPdfFontManagerBase::RegisterFontCollection: ")) +
...
wxPdfFontManagerBase::RegisterFontCJK(const wxString& family)
{
  ...
  {
    wxString fontFileName = family.Lower() + wxString(wxT_2(".xml"));
    wxString fullFontFileName;
    if (FindFile(fontFileName, fullFontFileName))
    {
      ok = RegisterFontCJK(fullFontFileName, wxT_2(""), family);
      if (ok)
      {
        // Add all available styles (bold, italic and bold-italic)
        // For all styles the same font metric file is used.
        RegisterFontCJK(fullFontFileName, wxT_2(",Bold"), family);
        RegisterFontCJK(fullFontFileName, wxT_2(",Italic"), family);
        RegisterFontCJK(fullFontFileName, wxT_2(",BoldItalic"), family);
      }
    }
    else
    {
      wxLogError(wxString(wxT_2("wxPdfFontManagerBase::RegisterFontCJK: ")) +
                 wxString::Format(_("CJK Font file '%s' for CJK family '%s' does not exist or is not readable."), fontFileName.c_str(), family.c_str()));
    }
  }
  else
  {
    wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::RegisterFontCJK: ")) +
...
wxPdfFontManagerBase::RegisterSystemFonts()
{
  ...
  {
    strFont = wxT_2("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts");
  }
  else
  {
    strFont = wxT_2("HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Fonts");
  }
  ...
  for (i = 0; i < nValues; ++i)
  {
    ...
    if (fontFileName.Find(wxT_2('\\')) == wxNOT_FOUND)
    {
      fontFileName.Prepend(fontDirectory);
    }
    if (fontFileName.Lower().EndsWith(wxT_2(".ttc")))
    {
...
// TODO: Would testing for __WXGTK__ be sufficient?
  ...
  if (fc == NULL)
  {
    wxLogError(wxString(wxT_2("wxPdfFontManagerBase::RegisterSystemFonts: ")) +
               wxString(_("FontConfig init failed.")));
    return 0;
  }
#endif
...
#elif defined(__WXMAC__)
  count += RegisterFontDirectory(wxT_2("/System/Library/Fonts"));
  count += RegisterFontDirectory(wxT_2("/Library/Fonts"));
#else
  wxLogWarning(wxString(wxT_2("wxPdfFontManagerBase::RegisterSystemFonts: ")) +
...
public:
  ...
  virtual wxDirTraverseResult OnFile(const wxString& fileName)
  {
    ...
    if (ext.IsSameAs(wxT_2("ttf")) || ext.IsSameAs(wxT_2("otf")) ||
        /* ext.IsSameAs(wxT_2("pfa")) || */ ext.IsSameAs(wxT_2("pfb")))
    {
    ...
    else if (ext.IsSameAs(wxT_2("ttc")))
    {
...
wxPdfFontManagerBase::RegisterFontDirectory(const wxString& directory, bool recursive)
{
  ...
  if (wxDir::Exists(directory))
  {
    ...
    {
      wxLogWarning(wxString(wxT_2("wxPdfFontManagerBase::RegisterFontDirectory: ")) +
...
wxPdfFontManagerBase::GetFont(const wxString& fontName, int fontStyle) const
{
  ...
  if (fontData == NULL)
  {
    ...
    else
    {
      ...
      wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::GetFont: ")) +
...
wxPdfFontManagerBase::GetFont(const wxString& fontName, const wxString& fontStyle) const
{
  ...
  {
    if (localStyle.Find(wxT_2("bold")) != wxNOT_FOUND)
    {
      style |= wxPDF_FONTSTYLE_BOLD;
    }
    if (localStyle.Find(wxT_2("italic")) != wxNOT_FOUND || localStyle.Find(wxT_2("oblique")) != wxNOT_FOUND)
    {
  ...
  {
    if (localStyle.Find(wxT_2("b")) != wxNOT_FOUND)
    {
      style |= wxPDF_FONTSTYLE_BOLD;
    }
    if (localStyle.Find(wxT_2("i")) != wxNOT_FOUND)
    {
      style |= wxPDF_FONTSTYLE_ITALIC;
    }
  }
...
wxPdfFontManagerBase::SetFontBaseEncoding(wxPdfFontData* fontData)
{
  if (fontData != NULL)
  {
    ...
    if (encoding.IsEmpty())
    {
      encoding = wxT_2("iso-8859-1");
    }
    if (fontType.IsSameAs(wxT_2("TrueType")) || fontType.IsSameAs(wxT_2("Type1")))
    {
    ...
    else if (fontType.IsSameAs(wxT_2("Type0")))
    {
...
wxPdfFontManagerBase::InitializeCoreFonts()
{
  ...
  if (!RegisterEncoding(wxT_2("winansi")) ||
      !RegisterEncoding(wxT_2("iso-8859-1")))
  {
    wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::InitializeCoreFonts: ")) +
  ...
  for (j = 0; gs_coreFontTable[j].name != wxEmptyString; ++j)
  {
    ...
    bool isWinAnsi = !(family.IsSameAs(wxT_2("Symbol")) || family.IsSameAs(wxT_2("ZapfDingbats")));
    wxString encoding = (isWinAnsi) ? wxT_2("winansi") : wxT_2("iso-8859-1");
...
wxPdfFontManagerBase::RegisterEncoding(const wxString& encoding)
{
  ...
  if (m_encodingMap->find(encodingName) == m_encodingMap->end())
  {
    ...
    {
      wxLogDebug(wxString(wxT_2("wxPdfFontManagerBase::RegisterEncoding: ")) +
...
wxPdfFontManagerBase::LoadFontFromXML(const wxString& fontFileName)
{
  ...
  if (xmlFontMetrics != NULL)
  {
    ...
    if (loaded)
    {
      if (fontMetrics.IsOk() && fontMetrics.GetRoot()->GetName().IsSameAs(wxT_2("wxpdfdoc-font-metrics")))
      {
        wxString fontType;
        wxXmlNode* root = fontMetrics.GetRoot();
#if wxCHECK_VERSION(2,9,0)
        if (root->GetAttribute(wxT_2("type"), &fontType))
#else
        if (root->GetPropVal(wxT_2("type"), &fontType))
#endif
        {
          if (fontType.IsSameAs(wxT_2("TrueType")))
          {
            fontData = new wxPdfFontDataTrueType();
          }
          else if (fontType.IsSameAs(wxT_2("Type1")))
          {
            fontData = new wxPdfFontDataType1();
          }
#if wxUSE_UNICODE
          else if (fontType.IsSameAs(wxT_2("TrueTypeUnicode")))
          {
            fontData = new wxPdfFontDataTrueTypeUnicode();
          }
          else if (fontType.IsSameAs(wxT_2("OpenTypeUnicode")))
          {
          ...
          {
            // Unknown font type
            wxLogError(wxString(wxT_2("wxPdfFontManagerBase::LoadFontFromXML: ")) +
...
wxPdfFontManagerBase::AddFont(wxPdfFontData* fontData, wxPdfFont& font)
{
  ...
  if (!alias.IsEmpty() && !alias.IsSameAs(family))
  {
    ...
    if (aliasIter != m_fontAliasMap.end())
    {
      ...
      {
        wxLogError(wxString(wxT_2("wxPdfFontManagerBase::AddFont: ")) +
