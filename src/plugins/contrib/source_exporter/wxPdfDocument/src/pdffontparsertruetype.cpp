...
ExtractFontData(HDC hdc, DWORD& fontDataSize, BYTE*& fontData)
{
  ...
  if (GetFontData(hdc, TTC_FILE, 0, NULL, 0) != GDI_ERROR)
  {
    ...
    {
      wxLogError(wxString(wxT_2("ExtractFontData: ")) +
...
static const wxChar* tableNamesDefault[] = {
  wxT_2("cvt "), wxT_2("fpgm"), wxT_2("glyf"), wxT_2("head"),
  wxT_2("hhea"), wxT_2("hmtx"), wxT_2("loca"), wxT_2("maxp"), wxT_2("prep"),
  NULL
};
#endif
/* C::B end */
...
wxPdfFontParserTrueType::GetCollectionFontCount(const wxString& fontFileName)
{
  ...
  if (fontFile != NULL)
  {
    ...
    if (fileName.GetExt().Lower().IsSameAs(wxT_2("ttc")))
    {
      wxString mainTag = ReadString(4);
      if (mainTag == wxT_2("ttcf"))
      {
        SkipBytes(4);
        count = ReadInt();
      }
    ...
    if (fileName.GetExt().Lower().IsSameAs(wxT_2("ttc")))
    {
      if (fontIndex >= 0)
      {
        ...
        if (mainTag == wxT_2("ttcf"))
        {
          ...
          {
            wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::IdentifyFont: ")) +
    ...
    if (m_inFont != NULL)
    {
      ...
      if (fontIndex >= 0)
      {
        ...
        if (mainTag == wxT_2("ttcf"))
        {
          ...
          {
            wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::LoadFontData: ")) +
      ...
      if (ok)
      {
        if (ReadTableDirectory())
        {
          if (CheckTables())
          {
            ...
            if (m_cff)
            {
              ok = fontData->GetType().IsSameAs(wxT_2("OpenTypeUnicode"));
            }
            else
            {
              ok = fontData->GetType().IsSameAs(wxT_2("TrueTypeUnicode"));
            }
            if (ok)
            {
...
wxPdfFontParserTrueType::ReadTableDirectory()
{
  ...
  if (!m_isMacCoreText)
  {
    ...
    else
    {
      if (!m_fileName.IsEmpty())
      {
        wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::ReadTableDirectory: '")) +
...
{
  bool ok = true;
  int maxTableCount = (m_tableDirectory->find(wxT_2("CFF ")) == m_tableDirectory->end()) ? 8 : 6;
  int tableCount = 0;
...
wxPdfFontParserTrueType::CheckCff()
{
  ...
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("CFF "));
...
{
  wxPdfTableDirectoryEntry* tableLocation;
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("OS/2"));
  if (entry != m_tableDirectory->end())
  {
    tableLocation = entry->second;
    LockTable(wxT_2("OS/2"));
...
wxPdfFontParserTrueType::GetBaseFont()
{
  ...
  wxPdfTableDirectoryEntry* tableLocation;
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("name"));
  if (entry != m_tableDirectory->end())
  {
    tableLocation = entry->second;
    LockTable(wxT_2("name"));
    ...
    if (fontName.IsEmpty())
    {
      ...
      fontName.Replace(wxT_2(" "), wxT_2("-"));
    }
    ReleaseTable();
  }
  else
  {
    wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::GetBaseFont: "))+
...
wxPdfFontParserTrueType::ReadMaps()
{
  ...
  wxPdfTableDirectoryEntry* tableLocation;
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("head"));
  if (entry == m_tableDirectory->end())
  {
    wxLogError(wxString(wxT_2("wxPdfFontParser::ReadMaps: ")) +
               wxString::Format(_("Table 'head' does not exist in '%s,%s'."), m_fileName.c_str(), m_style.c_str()));
    return false;
  }
  tableLocation = entry->second;
  LockTable(wxT_2("head"));
  ...
  entry = m_tableDirectory->find(wxT_2("hhea"));
  if (entry == m_tableDirectory->end())
  {
    wxLogError(wxString(wxT_2("wxPdfFontParser::ReadMaps: ")) +
  ...
  entry = m_tableDirectory->find(wxT_2("OS/2"));
  if (entry != m_tableDirectory->end())
  {
    tableLocation = entry->second;
    LockTable(wxT_2("OS/2"));
  ...
  double italicAngle;
  entry = m_tableDirectory->find(wxT_2("post"));
  if (entry == m_tableDirectory->end())
  {
  ...
  wxString fbb = wxString::Format(wxT_2("[%d %d %d %d]"),
  ...
  entry = m_tableDirectory->find(wxT_2("cmap"));
  if (entry == m_tableDirectory->end())
  {
    wxLogError(wxString(wxT_2("wxPdfFontParser::ReadMaps: ")) +
  ...
  if (!ok)
  {
    wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::ReadMaps: ")) +
...
{
  wxPdfTableDirectoryEntry* tableLocation;
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("hmtx"));
  if (entry == m_tableDirectory->end())
  {
    wxLogError(wxString(wxT_2("wxPdfFontParser::ReadGlyphWidths: ")) +
               wxString::Format(_("Table 'hmtx' does not exist in '%s,%s'."), m_fileName.c_str(), m_style.c_str()));
    return false;
  }
  tableLocation = entry->second;
  LockTable(wxT_2("hmtx"));
  m_inFont->SeekI(tableLocation->m_offset);
...
wxPdfFontParserTrueType::ReadFormat4()
{
  ...
  for (k = 0; k < segCount; k++)
  {
    ...
    for (j = startCount[k]; j <= endCount[k] && j != 0xFFFF; j++)
    {
      ...
      (*h)[idx] = r;
//      wxLogMessage(wxT_2("C %ld G %ld"), idx, glyph);
    }
  }
...
{
  wxPdfTableDirectoryEntry* tableLocation;
  wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxT_2("kern"));
  if (entry != m_tableDirectory->end())
  {
    tableLocation = entry->second;
    LockTable(wxT_2("kern"));
...
wxPdfFontParserTrueType::GetNames(int id, bool namesOnly)
{
  ...
  if (entry != m_tableDirectory->end())
  {
    ...
    for (k = 0; k < numRecords; k++)
    {
      ...
      if (nameID == id)
      {
        ...
        if (!namesOnly)
        {
          names.Add(wxString::Format(wxT_2("%d"), platformID));
          names.Add(wxString::Format(wxT_2("%d"), platformEncodingID));
          names.Add(wxString::Format(wxT_2("%d"), languageID));
        }
        names.Add(name);
        m_inFont->SeekI(pos);
      }
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfFontParser::GetNames: ")) +
...
wxPdfFontParserTrueType::GetEnglishName(int id)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfFontParserTrueType::GetEnglishName: ")) +
               wxString::Format(_("Table 'name' does not exist in '%s,%s'."), m_fileName.c_str(), m_style.c_str()));
  }
  return englishName;
}
