...
wxPdfDocument::OutIndirectObject(wxPdfIndirectObject* object)
{
  ...
  if (m_offsets->count(objectId-1) == 0)
  {
    ...
    OutAscii(wxString::Format(wxT_2("%d %d obj"), objectId, generationId));
...
// Currently radio button groups do not function
          Out("<<");
          Out("/FT /Btn");
          OutAscii(wxString::Format(wxT_2("/Ff %d"), (1 << 14) | (1 << 15)));
//          Out("/F 4");
          Out("/T ", false);
          OutAsciiTextstring(obj->GetName());
#if 0
          OutAscii(wxString(wxT_2("/BS << /W ") + 
                   Double2String(obj->GetBorderWidth() +
                   wxString(wxT_2("/S/")) + obj->GetBorderStyle() + 
                   wxString(wxT_2(">>")));
#endif
//          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorder() +
//                   wxString(wxT_2("]/BG [")) + obj->GetBackground() + wxString(wxT_2("] /CA ")), false);
//          OutAsciiTextstring(wxString(wxT_2("4")), false);
//          Out(">>");
          Out("/DR 2 0 R");
          Out("/DA ", false);
//          OutAsciiTextstring(wxString::Format(wxT_2("/F%d 9 Tf "), m_zapfdingbats) + obj->GetTextColour());
//          Out("/AP <</N <</Yes <<>>>> >>");
//          Out("/AS /Off");
          Out("/V /V1");
          Out("/Kids [", false);
          ...
          for (j = 0; j < kids.GetCount(); j++)
          {
            ...
            OutAscii(wxString::Format(wxT_2("%d %d R "), radio->GetObjectId(), radio->GetGenerationId()), false);
          }
          Out("]");
          Out(">>");
          Out("endobj");
      ...
      case wxPDF_OBJECT_WIDGET_CHECKBOX:
        {
          ...
          OutAscii(wxString(wxT_2("/Rect [")) +
                   wxPdfUtility::Double2String(obj->GetX()*m_k,2) + wxString(wxT_2(" ")) +
                   wxPdfUtility::Double2String(yPos*m_k,2) + wxString(wxT_2(" ")) +
                   wxPdfUtility::Double2String((obj->GetX() + obj->GetWidth())*m_k,2) +  wxString(wxT_2(" ")) +
                   wxPdfUtility::Double2String((yPos - obj->GetHeight())*m_k,2) + wxString(wxT_2("]")));
          Out("/FT /Btn");
          Out("/Ff 0");
          Out("/F 4");
          Out("/T ", false);
          ...
          OutAscii(wxString(wxT_2("/BS << /W ")) + 
                   ...
                   wxString(wxT_2("/S/")) + obj->GetBorderStyle() + 
                   wxString(wxT_2(">>")));
          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorderColour() +
                   wxString(wxT_2("]/BG [")) + obj->GetBackgroundColour() + wxString(wxT_2("] /CA ")), false);
          OutAsciiTextstring(wxString(wxT_2("4")), false);
          Out(">>");
          Out("/DR 2 0 R");
          Out("/DA ", false);
      ...
      case wxPDF_OBJECT_WIDGET_COMBOBOX:
        {
          ...
          OutAsciiTextstring(obj->GetName());
          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorderColour() +
                   wxString(wxT_2("]/BG [")) + obj->GetBackgroundColour() + wxString(wxT_2("]>>")));
          // Border style
          OutAscii(wxString(wxT_2("/BS << /W ")) + 
                   wxPdfUtility::Double2String(obj->GetBorderWidth(), 2) +
                   wxString(wxT_2("/S/")) + obj->GetBorderStyle() + 
          ...
          Out("/DR 2 0 R");
          Out("/DA ", false);
          OutAsciiTextstring(wxString::Format(wxT_2("/F%d "), obj->GetFontIndex()) +
                             wxPdfUtility::Double2String(obj->GetFontSize(),2) +
                             wxString(wxT_2(" Tf ")) + obj->GetTextColour()); 
...
//        Out("/H");
          ...
          Out("/BS << /W 1 /S /B >>");
          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorderColour() +
                   wxString(wxT_2("]/BG [")) + obj->GetBackgroundColour() + 
                   wxString(wxT_2("] /CA ")), false);
...
// TODO: integrate radio button groups into PDF document
          OutAscii(wxString::Format(wxT_2("/Parent [%d %d R]"), parent->GetObjectId(), parent->GetGenerationId()));
//          Out("/P ??? 0 R /H /T ");
#endif
          Out("/T ", false);
          ...
          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorderColour() +
                   wxString(wxT_2("]/BG [")) + obj->GetBackgroundColour() + 
                   wxString(wxT_2("] /CA ")), false);
          OutAsciiTextstring(wxString(wxT_2("l")), false);
          Out(">>");
          // Border style
          OutAscii(wxString(wxT_2("/BS << /W ")) + 
                   wxPdfUtility::Double2String(obj->GetBorderWidth(), 2) +
          ...
          Out("/DA ", false);
          OutAsciiTextstring(wxString::Format(wxT_2("/F%d 9 Tf "), m_zapfdingbats) + obj->GetTextColour());
          OutAscii(wxString::Format(wxT_2("/AP <</N <</V%d <<>> >> >>"), obj->GetIndex()));
          // Set first button in group as selected
          if (obj->GetIndex() == 1)
          {
            OutAscii(wxString::Format(wxT_2("/V /V%d"), obj->GetIndex()));
          }
          Out(">>");
          Out("endobj");
        }
...
//          OutAscii(wxString::Format(wxT_2("/MaxLen %d"), obj->GetMaxLength());
          OutAscii(wxString(wxT_2("/MK <</BC [")) + obj->GetBorderColour() +
                   wxString(wxT_2("]/BG [")) + obj->GetBackgroundColour() + wxString(wxT_2("]>>")));
          // Border style
          OutAscii(wxString(wxT_2("/BS << /W ")) + 
...
{
  m_formBorderColour     = borderColour.GetColour(false).BeforeLast(wxT_2(' '));
  m_formBackgroundColour = backgroundColour.GetColour(false).BeforeLast(wxT_2(' '));
...
wxPdfDocument::SetFormBorderStyle(wxPdfBorderStyle borderStyle, double borderWidth)
{
  switch (borderStyle)
  {
    case wxPDF_BORDER_DASHED:    m_formBorderStyle = wxString(wxT_2("D")); break;
    case wxPDF_BORDER_BEVELED:   m_formBorderStyle = wxString(wxT_2("B")); break;
    case wxPDF_BORDER_INSET:     m_formBorderStyle = wxString(wxT_2("I")); break;
    case wxPDF_BORDER_UNDERLINE: m_formBorderStyle = wxString(wxT_2("U")); break;
    case wxPDF_BORDER_SOLID:
    default:                     m_formBorderStyle = wxString(wxT_2("S")); break;
  }
...
wxPdfDocument::LoadZapfDingBats()
{
  if (m_zapfdingbats == 0)
  {
    ...
    SelectFont(wxT_2("ZapfDingBats"), wxT_2(""), 9, false);
