...
wxPdfDocument::SetScaleFactor(const wxString& unit)
{
  ...
  else if (unit == wxT_2("in"))
  {
    m_k = 72.;
  }
  else if (unit == wxT_2("cm"))
  {
    m_k = 72. / 2.54;
  }
  ...
  if (m_encryptor == NULL)
  {
    ...
    switch (encryptionMethod)
    {
      case wxPDF_ENCRYPTION_AESV2:
        revision = 4;
        if (m_PDFVersion < wxT_2("1.6"))
        {
          m_PDFVersion = wxT_2("1.6");
        }
        break;
    ...
    {
      ownerPswd = wxPdfUtility::GetUniqueId(wxT_2("wxPdfDoc"));
    }
...
wxPdfDocument::AddPage(int orientation, double pageWidth, double pageHeight)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::AddPage: ")) +
...
wxPdfDocument::AddPage(int orientation, wxSize pageSize)
{
  ...
  // Set line width
  m_lineWidth = lw;
  OutAscii(wxPdfUtility::Double2String(lw*m_k,2)+wxString(wxT_2(" w")));
...
wxPdfDocument::SetLineWidth(double width)
{
  ...
  if (m_page > 0)
  {
    OutAscii(wxPdfUtility::Double2String(width*m_k,2)+ wxString(wxT_2(" w")));
  }
}
...
wxPdfDocument::AddFont(const wxString& family, const wxString& style, const wxString& file)
{
  ...
  if (ok)
  {
    ...
    if (!regFont.IsValid())
    {
      ...
      if (fileName.IsEmpty())
      {
        fileName = family.Lower() + style.Lower() + wxString(wxT_2(".xml"));
        fileName.Replace(wxT_2(" "),wxT_2(""));
      }
...
wxPdfDocument::SetFontSize(double size)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetFontSize: ")) +
...
wxPdfDocument::GetCurrentFont() const
{
  if (m_currentFont == NULL)
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::GetCurrentFont: ")) +
               wxString(_("No font selected.")));
    return wxPdfFont();
  }
...
wxPdfDocument::GetFontStyle() const
{
  ...
  {
    style += wxString(wxT_2("O"));
  }
  if (styles & wxPDF_FONTSTYLE_STRIKEOUT)
  {
    style += wxString(wxT_2("S"));
  }
  return style;
}
...
wxPdfDocument::Text(double x, double y, const wxString& txt)
{
  ...
  if (m_yAxisOriginTop)
  {
    OutAscii(wxString(wxT_2("BT 1 0 0 -1 ")) +
             wxPdfUtility::Double2String(x*m_k,2) + wxString(wxT_2(" ")) +
             wxPdfUtility::Double2String(y*m_k,2) + wxString(wxT_2(" Tm ")), false);
  }
  else
  {
    OutAscii(wxString(wxT_2("BT ")) +
             wxPdfUtility::Double2String(x*m_k,2) + wxString(wxT_2(" ")) +
             wxPdfUtility::Double2String(y*m_k,2) + wxString(wxT_2(" Td ")), false);
  }
  OutAscii(wxString::Format(wxT_2("%d Tr "), m_textRenderMode), false);
  ShowText(voText);
  Out("ET", false);
...
wxPdfDocument::DoCell(double w, double h, const wxString& txt, int border, int ln, int align, int fill, const wxPdfLink& link)
{
  ...
  if (doPageBreak && !m_inFooter && AcceptPageBreak())
  {
    ...
    {
      m_ws = ws;
      OutAscii(wxPdfUtility::Double2String(ws*k,3)+wxString(wxT_2(" Tw")));
    }
  }
  if ( w == 0)
  {
  ...
  {
    s = wxPdfUtility::Double2String(m_x*k,2) + wxString(wxT_2(" ")) +
        wxPdfUtility::Double2String(m_y*k,2) + wxString(wxT_2(" ")) +
        wxPdfUtility::Double2String(w*k,2) + wxString(wxT_2(" ")) +
        wxPdfUtility::Double2String(h*k,2);
    if (fill == 1)
    {
      if (border == wxPDF_BORDER_FRAME)
      {
        s += wxString(wxT_2(" re B "));
      }
      else
      {
        s += wxString(wxT_2(" re f "));
      }
    }
    else
    {
      s += wxString(wxT_2(" re S "));
    }
  }
  if (border != wxPDF_BORDER_NONE && border != wxPDF_BORDER_FRAME)
  {
    ...
    {
      s += wxPdfUtility::Double2String(x*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(y*k,2) + wxString(wxT_2(" m ")) +
           wxPdfUtility::Double2String(x*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((y+h)*k,2) + wxString(wxT_2(" l S "));
    }
    if (border & wxPDF_BORDER_TOP)
    {
      s += wxPdfUtility::Double2String(x*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(y*k,2) + wxString(wxT_2(" m ")) +
           wxPdfUtility::Double2String((x+w)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(y*k,2) + wxString(wxT_2(" l S "));
    }
    if (border & wxPDF_BORDER_RIGHT)
    {
      s += wxPdfUtility::Double2String((x+w)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(y*k,2) + wxString(wxT_2(" m ")) +
           wxPdfUtility::Double2String((x+w)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((y+h)*k,2) + wxString(wxT_2(" l S "));
    }
    ...
    {
      s += wxPdfUtility::Double2String(x*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((y+h)*k,2) + wxString(wxT_2(" m ")) +
           wxPdfUtility::Double2String((x+w)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((y+h)*k,2) + wxString(wxT_2(" l S "));
    }
  }
  ...
  if (txt.Length() > 0)
  {
    ...
    if (m_colourFlag)
    {
      s += wxString(wxT_2("q ")) + m_textColour.GetColour(false) + wxString(wxT_2(" "));
    }
    if (m_yAxisOriginTop)
    {
      s += wxString(wxT_2("BT 1 0 0 -1 ")) +
           wxPdfUtility::Double2String((m_x+dx)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((m_y+.5*h+.3*m_fontSize)*k,2) + wxString(wxT_2(" Tm "));
    }
    else
    {
      s += wxString(wxT_2("BT ")) +
           wxPdfUtility::Double2String((m_x+dx)*k,2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String((m_y+.5*h+.3*m_fontSize)*k,2) + wxString(wxT_2(" Td "));
    }
    OutAscii(s,false);
    OutAscii(wxString::Format(wxT_2("%d Tr "), m_textRenderMode), false);
    ShowText(txt);
    ...
    {
      s += wxString(wxT_2(" ")) + DoDecoration(m_x+dx,m_y+.5*h+.3*m_fontSize,txt);
    }
    if (m_colourFlag)
    {
      s += wxString(wxT_2(" Q"));
...
wxPdfDocument::WriteGlyphArray(wxPdfArrayDouble& x, wxPdfArrayDouble& y, wxPdfArrayUint32& glyphs)
{
  ...
  if (ok)
  {
    ...
    if (fontType.IsSameAs(wxT_2("TrueTypeUnicode")) || fontType.IsSameAs(wxT_2("OpenTypeUnicode")))
    {
    ...
    {
      ok = false;
      wxLogError(wxString(wxT_2("wxPdfDocument::WriteGlyphArray: ")) +
  ...
  if (img.Ok())
  {
    ...
    if (image == (*m_images).end())
    {
      if (tempImage.HasAlpha())
      {
        if (maskImage <= 0)
        {
          maskImage = ImageMask(name+wxString(wxT_2(".mask")), tempImage);
        }
      ...
      else if (tempImage.HasMask() && maskImage <= 0)
      {
        ...
        maskImage = ImageMask(name+wxString(wxT_2(".mask")), mask);
      }
      // First use of image, get info
...
wxPdfDocument::ImageMask(const wxString& file, const wxString& type)
{
  ...
  if (image == (*m_images).end())
  {
    ...
    if (currentImage->GetColourSpace() != wxT_2("DeviceGray"))
    {
      delete currentImage;
      return 0;
    }
...
wxPdfDocument::SetViewerPreferences(int preferences)
{
  ...
  if (((m_viewerPrefs & wxPDF_VIEWER_DISPLAYDOCTITLE) != 0) && (m_PDFVersion < wxT_2("1.4")))
  {
    m_PDFVersion = wxT_2("1.4");
  }
}
...
wxPdfDocument::AttachFile(const wxString& fileName, const wxString& attachName, const wxString& description)
{
  ...
  {
    wxLogDebug(wxT_2("*** Attachment file '%s' does not exist."), fileName.c_str());
  }
  return ok;
}
...
wxPdfDocument::AddPattern(const wxString& patternName, const wxImage& image, double width, double height)
{
  ...
  if (patternIter == m_patterns->end())
  {
    ...
    {
      wxString imageName = wxString(wxT_2("pattern:")) + patternName;
      wxPdfImage* currentImage = NULL;
      ...
      if (imageIter == (*m_images).end())
      {
        ...
        if (tempImage.HasAlpha())
        {
          maskImage = ImageMask(imageName+wxString(wxT_2(".mask")), tempImage);
    ...
    else
    {
      ...
      if (!image.IsOk())
      {
        wxLogError(wxString(wxT_2("wxPdfDocument::AddPattern: ")) +
...
wxPdfDocument::SetDrawColour(const wxString& name, double tint)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetDrawColour: ")) +
...
wxPdfDocument::SetDrawPattern(const wxString& name)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetDrawPattern: ")) +
...
wxPdfDocument::SetFillColour(const wxString& name, double tint)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetFillColour: ")) +
               wxString::Format(_("Undefined spot colour: '%s'."), name.c_str()));
  }
}
...
wxPdfDocument::SetFillPattern(const wxString& name)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetFillPattern: ")) +
               wxString::Format(_("Undefined pattern: '%s'."), name.c_str()));
  }
}
...
wxPdfDocument::GetPatternColour(const wxString& name)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::GetPatternColour: ")) +
               wxString::Format(_("Undefined pattern: '%s'."), name.c_str()));
  }
  return colour;
}
...
wxPdfDocument::SetTextColour(const wxString& name, double tint)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::SetTextColour: ")) +
               wxString::Format(_("Undefined spot colour: '%s'."), name.c_str()));
  }
}
