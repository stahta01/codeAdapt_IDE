...
wxPdfDocument::UseTemplate(int templateId, double x, double y, double w, double h)
{
  if (m_page <= 0)
  {
    wxLogError(wxString(wxT_2("wxPdfDocument::UseTemplate: ")) +
  ...
  {
    wxLogWarning(wxString(wxT_2("wxPdfDocument::UseTemplate: ")) +
  ...
  OutAscii(wxString(wxT_2("q ")) +
           wxPdfUtility::Double2String(xScale, 4) + wxString(wxT_2(" 0 0 ")) +
           wxPdfUtility::Double2String(yScale, 4) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(xTrans, 2) + wxString(wxT_2(" ")) +
           wxPdfUtility::Double2String(yTrans, 2) +  wxString(wxT_2(" cm"))); // Translate
  OutAscii(m_templatePrefix + wxString::Format(wxT_2("%d Do Q"), tpl->GetIndex()));
  tpl->m_used = true;
}
...
wxPdfDocument::GetTemplateBBox(int templateId, double& x, double& y, double& w, double& h)
{
  ...
  else
  {
    ...
    w = 0;
    h = 0;
    wxLogWarning(wxString(wxT_2("wxPdfDocument::GetTemplateBBox: ")) +
                 wxString::Format(_("Template %d does not exist!"), templateId));
  }
}
...
wxPdfDocument::SetTemplateBBox(int templateId, double x, double y, double w, double h)
{
  ...
  if (templateIter != (*m_templates).end())
  {
    ...
    if (!tpl->m_used)
    {
      ...
      {
        wxLogWarning(wxString(wxT_2("wxPdfDocument::SetTemplateBBox: ")) +
...
wxPdfDocument::SetSourceFile(const wxString& filename, const wxString& password)
{
  ...
  if (filename != wxEmptyString)
  {
    ...
    {
      wxLogError(wxString(wxT_2("wxPdfDocument::SetSourceFile: ")) +
  ...
  if (m_currentParser != NULL)
  {
    if (pageno > 0 && pageno <= m_currentParser->GetPageCount())
    {
      ...
      if (rotationAngle != 0)
      {
        ...
        double c = cos(angle);
        double s = sin(angle);
        wxString strRotation = wxString(wxT_2("q ")) + 
                               wxPdfUtility::Double2String(  c,3) + wxString(wxT_2(" ")) +
                               wxPdfUtility::Double2String(  s,3) + wxString(wxT_2(" ")) +
                               wxPdfUtility::Double2String( -s,3) + wxString(wxT_2(" ")) +
                               wxPdfUtility::Double2String(  c,3) + wxString(wxT_2(" ")) +
                               wxPdfUtility::Double2String( cx,3) + wxString(wxT_2(" ")) + 
                               wxPdfUtility::Double2String( cy,3) + wxString(wxT_2(" cm 1 0 0 1 ")) +
                               wxPdfUtility::Double2String(-cx,3) + wxString(wxT_2(" ")) +
                               wxPdfUtility::Double2String(-cy,3) + wxString(wxT_2(" cm "));
    ...
    case OBJTYPE_STREAM:
      {
        ...
        wxPdfObject* originalLength = dictionary->Get(wxT_2("Length"));
        ...
        wxPdfName lengthKey(wxT_2("Length"));
    ...
    case OBJTYPE_INDIRECT:
      {
        ...
        OutAscii(wxString::Format(wxT_2("%d 0 R"), actualObjectId), newline);
      }
      break;
