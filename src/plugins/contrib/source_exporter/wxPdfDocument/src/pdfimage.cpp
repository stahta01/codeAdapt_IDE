...
wxPdfImage::Parse()
{
  ...
  if (m_imageStream)
  {
    if ((m_type.StartsWith(wxT_2("image/")) && m_type.EndsWith(wxT_2("png"))) ||
        m_type == wxT_2("png"))
    {
      isValid = ParsePNG(m_imageStream);
    }
    else if ((m_type.StartsWith(wxT_2("image/")) && m_type.EndsWith(wxT_2("jpeg"))) ||
             m_type == wxT_2("jpeg") || m_type == wxT_2("jpg"))
    {
      isValid = ParseJPG(m_imageStream);
    }
    else if ((m_type.StartsWith(wxT_2("image/")) && m_type.EndsWith(wxT_2("gif"))) ||
             m_type == wxT_2("gif"))
    {
    ...
    {
      if ((m_type.StartsWith(wxT_2("image/")) && m_type.EndsWith(wxT_2("wmf"))) ||
          m_type == wxT_2("wmf") || m_name.Right(2) == wxT_2(".wmf"))
      {
        m_isFormObj = true;
...
wxPdfImage::ParsePNG(wxInputStream* imageStream)
{
  ...
  {
    // Not a PNG file
    wxLogDebug(wxString(wxT_2("wxPdfImage::ParsePNG: ")) +
  ...
  if (ct == 0)
  {
    colspace = wxT_2("DeviceGray");
  }
  else if (ct == 2)
  {
    colspace = wxT_2("DeviceRGB");
  }
  else if (ct == 3)
  {
    colspace = wxT_2("Indexed");
  }
  else
  {
    // Unknown colour type
  ...
  imageStream->Read(buffer,4);
  m_parms = wxString::Format(wxT_2("/DecodeParms <</Predictor 15 /Colors %d /BitsPerComponent %d /Columns %d>>"), (ct==2 ? 3 : 1), (int) bpc, w);
  ...
  if (colspace == wxT_2("Indexed") && m_pal == NULL)
  {
    if (m_pal  != NULL) delete [] m_pal;
    if (m_trns != NULL) delete [] m_trns;
    if (m_data != NULL) delete [] m_data;
...
wxPdfImage::ParseJPG(wxInputStream* imageStream)
{
  ...
  {
    // Not a JPEG file
    wxLogDebug(wxString(wxT_2("wxPdfImage::ParseJPG: ")) +
...
wxPdfImage::ParseGIF(wxInputStream* imageStream)
{
  ...
  {
    wxLogDebug(wxString(wxT_2("wxPdfImage::ParseGIF: ")) +
      ...
      case 0x012d:  // SelectObject
        {
          ...
          switch (obj->type)
          {
            case 'B':
              ...
              {
                data += wxPdfUtility::Double2String(obj->r/255.,3) + wxString(wxT_2(" "));
                data += wxPdfUtility::Double2String(obj->g/255.,3) + wxString(wxT_2(" "));
                data += wxPdfUtility::Double2String(obj->b/255.,3) + wxString(wxT_2(" rg\n"));
              }
              break;
            ...
            case 'P':
              ...
              if (!nullPen)
              {
                ...
                data += wxPdfUtility::Double2String(obj->width*m_document->m_k,2) + wxString(wxT_2(" w\n"));
              }
      ...
      case 0x0324: // Polygon
        {
          ...
          for (k = numpoints; k > 0; k--)
          {
            ...
            if (k < numpoints)
            {
              data += wxString::Format(wxT_2("%d %d l\n"), (int) px, (int) py);
            }
            else
            {
              data += wxString::Format(wxT_2("%d %d m\n"), (int) px, (int) py);
            }
          }
          ...
          else if (func == 0x0324)
          {
            ...
            else
            {
              ...
              {
                op = wxT_2("b");  // stroke and fill
              }
            }

            if (polyFillMode == 1 && (op == wxT_2("b") || op == wxT_2("f")))
            {
              op += wxT_2("*");  // use even-odd fill rule
            }
          }
          data += op + wxT_2("\n");
          delete [] coords;
