  ...
  while (child)
  {
    ...
    else if (child->GetName() == wxT_2("file"))
    {
      ...
      if (value.Length() > 0)
      {
        m_file = value;
#if wxCHECK_VERSION(2,9,0)
        value = child->GetAttribute(wxT_2("size1"), wxT_2(""));
#else
        value = child->GetPropVal(wxT_2("size1"), wxT_2(""));
#endif
        if (value.Length() > 0 && value.ToLong(&number))
        {
          ...
          m_size1 = number;
#if wxCHECK_VERSION(2,9,0)
          value = child->GetAttribute(wxT_2("size2"), wxT_2(""));
#else
          value = child->GetPropVal(wxT_2("size2"), wxT_2(""));
#endif
          if (value.Length() > 0 && value.ToLong(&number))
          {
            m_size2 = number;
...
wxPdfFontDataType1::GetWidthsAsString(const wxArrayString& glyphNames, bool subset, wxPdfSortedArrayInt* usedGlyphs, wxPdfChar2GlyphMap* subsetGlyphs) const
{
  ...
  for (i = 32; i <= 255; i++)
  {
    ...
    s += wxString::Format(wxT_2("%d "), width);
  }
  s += wxString(wxT_2("]"));
  return s;
}
...
wxPdfFontDataType1::CompressFontData(wxOutputStream* fontData, wxInputStream* pfbFile)
{
  ...
  if (size1 < 0 || size2 < 0)
  {
    wxLogError(wxString(wxT_2("wxPdfFontDataType1::CompressFontData: ")) + 
...
wxPdfFontDataType1::WriteFontData(wxOutputStream* fontData, wxPdfSortedArrayInt* usedGlyphs, wxPdfChar2GlyphMap* subsetGlyphs)
{
  ...
  if (m_pfbStream == NULL)
  {
    ...
    else
    {
      ...
      wxLogError(wxString(wxT_2("wxPdfFontDataType1::WriteFontData: ")) +
                                    ...
                                    wxPdfChar2GlyphMap* subsetGlyphs)
{
  ...
  {
    wxLogError(wxString(wxT_2("wxPdfFontDataType1::WriteUnicodeMap: ")) +
